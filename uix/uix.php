...
class fwpcl_uix{
	...
	public function create_admin_page(){
						...
						<?php 
							...
							if( !empty( $tab['template'] ) && file_exists( $tab['template'] ) ){
								...
								if( !empty( $tab['controls'] )){
									$controls[ $tab_slug ] = $tab['controls'];
									foreach( array_keys( $tab['controls'] ) as $part_key ){
		...
		<?php 
		if( !empty( $controls ) ){
			foreach( $controls as $control_slug=>$structure ){
				uix_render_control( $structure, $control_slug, '', true );
			}
		}
		?>
